function PeriodicTable(container) {
	this.container = container;

	this.containers = {
		"table": $("<div/>", {class: "section table"}).appendTo(this.container),
		"lanthanoids": $("<div/>", {class: "section radioactive"}).appendTo(this.container),
		"actinoids": $("<div/>", {class: "section radioactive"}).appendTo(this.container)
	};
	
	this.elements = [];
	for (key in PeriodicTable.ELEMENTS) {
		if (PeriodicTable.ELEMENTS.hasOwnProperty(key)) {
			PeriodicTable.ELEMENTS[key].forEach(function(period, idx) {
				var periodContainer = $("<div/>",{"class":"period"}).appendTo(this.containers[key]);
				period.elements.forEach(function(element) {
					var element = new Element(element);
					element.container.appendTo(periodContainer);
...
PeriodicTable.prototype = {
	bindToElements: function(eventType, callback) {
		this.elements.forEach(function(element){
			element.container.bind(eventType, function(ev){
				callback(element);
			});
		});
	}
